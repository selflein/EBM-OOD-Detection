...
from .bound_spectral_norm import spectral_norm_fc
from .bound_spectral_norm_conv import spectral_norm_conv
from .log_det import exact_matrix_logarithm_trace, power_series_matrix_logarithm_trace
...
def logistic_distribution(loc, log_scale):
    ...
    base_distribution = distributions.Uniform(
...
class conv_iresnet_block(nn.Module):
    def __init__(
        ...
        layers.append(
            self._wrapper_spectral_norm(
                nn.Conv2d(int_ch, int_ch, kernel_size=kernel_size2, padding=0),
                (int_ch, h, w),
                kernel_size2,
            )
        )
        ...
        layers.append(
            self._wrapper_spectral_norm(
                nn.Conv2d(int_ch, in_ch, kernel_size=kernel_size3, padding=1),
                (int_ch, h, w),
                kernel_size3,
            )
        )
    ...
    def _wrapper_spectral_norm(self, layer, shapes, kernel_size):
        if kernel_size == 1:
            ...
            return spectral_norm_fc(
                layer, self.coeff, n_power_iterations=self.n_power_iter
            )
        else:
            # use spectral norm based on conv, because bound not tight
            return spectral_norm_conv(
                layer, self.coeff, shapes, n_power_iterations=self.n_power_iter
            )
...
class multiscale_conv_iResNet(nn.Module):
    def __init__(
        ...
        assert (
            nClasses is not None or density_estimation
        ), "Must be either classifier or density estimator"
    ...
    def _make_prior(self, learn_prior):
        ...
        self.prior_mu = nn.Parameter(
            torch.zeros((dim,)).float(), requires_grad=learn_prior
        )
...
class iResNetConv(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert len(n_blocks) == len(n_strides) == len(n_channels)
        ...
        self.nClasses = nClasses
        # parameters for trace estimation
        self.numTraceSamples = n_trace_samples if density_estimation else 0
        self.numSeriesTerms = n_series_terms if density_estimation else 0
        self.n_power_iter = n_power_iter

        print("")
        print(" == Building iResNet %d == " % (sum(n_blocks) * 3 + 1))
    ...
    def _make_stack(
        ...
        block_list = nn.ModuleList()
        in_shapes = []
        for i, (int_dim, stride, blocks) in enumerate(
            zip(nChannels, nStrides, nBlocks)
        ):
            for j in range(blocks):
                ...
                block_list.append(
                    block(
                        ...
                        input_nonlin=(
                            i + j > 0
                        ),  # add nonlinearity to input for all but fist layer
                        coeff=coeff,
                        actnorm=actnorm,
                        n_power_iter=n_power_iter,
                        nonlin=nonlin,
...
if __name__ == "__main__":
    ...
    int_dim = 32
    sb = scale_block(
        steps, in_shape[1:], int_dim, True, 5, 1, True, 0.9, False, True, True
    )
